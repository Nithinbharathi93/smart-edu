  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      <div className="mb-10 text-center">
        <h1 className="text-3xl font-bold text-black mb-2">Create New Syllabus</h1>
      ...
      <div className="flex bg-slate-100 p-1 rounded-xl mb-8">
        <button 
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-lg font-bold transition-all ${mode === 'topic' ? 'bg-white shadow-sm text-green-600' : 'text-slate-500 hover:text-slate-700'}`}
        >
          <Type size={18} /> Topic-Based
        </button>
        <button 
          onClick={() => setMode('pdf')}
          className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-lg font-bold transition-all ${mode === 'pdf' ? 'bg-white shadow-sm text-green-600' : 'text-slate-500 hover:text-slate-700'}`}
        >
      ...
      <form onSubmit={handleCreate} className="space-y-6 bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
        {mode === 'topic' ? (
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2 text-green-600">Learning Topic</label>
            <input 
              type="text"
          ...
          <div className="border-2 border-dashed border-slate-200 rounded-2xl p-10 text-center hover:border-green-500 transition-colors bg-slate-50/50">
            <input 
              type="file" 
              id="pdf-upload" 
              className="hidden" 
            ...
            <label htmlFor="pdf-upload" className="cursor-pointer">
              ...
              <p className="font-bold text-black">{file ? file.name : 'Click to upload PDF'}</p>
              <p className="text-xs text-green-600/70 mt-1">Maximum size: 10MB</p>
            </label>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="flex items-center gap-2 text-sm font-bold text-slate-700 mb-2">
              <BarChart3 size={16} className="text-green-600" /> Target Level
            </label>
            <select 
              className="w-full px-4 py-3 rounded-xl border border-slate-200 outline-none"
