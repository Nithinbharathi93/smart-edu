...
const LevelBreaker = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        setLoading(true);
        // GET /assessment/questions returns an array of question objects
  ...
  if (result) {
    return (
      <div className="max-w-md mx-auto mt-20 text-center p-10 bg-white rounded-[2.5rem] shadow-2xl shadow-indigo-100 border border-slate-50">
        <div className="w-24 h-24 bg-gradient-to-tr from-indigo-600 to-indigo-400 text-white rounded-full flex items-center justify-center mx-auto mb-8 shadow-lg shadow-indigo-200">
          <Trophy size={48} />
        </div>
        <h2 className="text-3xl font-black text-slate-900 mb-3 tracking-tight">Level Unlocked!</h2>
        ...
        <div className="bg-slate-50 rounded-3xl p-6 mb-10 border border-slate-100">
          <span className="text-xs font-bold text-indigo-600 uppercase tracking-widest block mb-2">New Global Rank</span>
          <div className="text-4xl font-black text-slate-900 tracking-tighter">{result.name}</div>
          <p className="text-xs text-slate-400 mt-2">Level {result.level} of 6</p>
        </div>
    ...
    <div className="max-w-3xl mx-auto mt-16 px-6 pb-20">
      ...
      <div className="mb-12">
        <div className="flex justify-between items-end mb-4">
          <div>
            <span className="px-3 py-1 bg-indigo-50 text-indigo-600 rounded-full font-bold text-xs uppercase tracking-wider">
              Assessment In Progress
            </span>
            <h2 className="text-3xl font-bold text-slate-900 mt-3">Question {currentIdx + 1}</h2>
          </div>
          <span className="text-slate-400 font-mono text-sm">{currentIdx + 1} / {questions.length}</span>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4">
          {q?.options.map((opt, i) => (
            <button 
              ...
              className="group flex items-center justify-between p-5 text-left border-2 border-slate-100 rounded-2xl hover:border-indigo-500 hover:bg-indigo-50/50 transition-all"
            >
